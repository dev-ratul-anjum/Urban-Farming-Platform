...
export const createProduceSchema = z.object({
  ...
  availableQuantity: z.preprocess(
    ...
    z.number().int().positive("Available quantity must be positive"),
  ),

  // Sustainability Cert embedded data
  certifyingAgency: z
    .string("Certifying agency is required")
    .min(1, "Certifying agency cannot be empty"),
  certificationDate: z
    .string("Certification date is required")
...
export const updateProduceSchema = z.object({
  ...
  price: z.preprocess(
    (a) => (a ? parseFloat(z.any().parse(a)) : undefined),
    z.number().positive("Price must be positive").optional(),
  ),
  category: z.string().min(1, "Category cannot be empty").optional(),
  availableQuantity: z.preprocess(
    (a) => (a ? parseInt(z.any().parse(a), 10) : undefined),
    z.number().int().positive("Available quantity must be positive").optional(),
  ),
});
